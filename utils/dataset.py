            ...
            if self.dataset_type in [1, 4]:
                ...
                for path in tqdm(track_paths):
                    ...
                    for instr in instruments:
                        ...
                        for extension in self.file_types:
                            path_to_audio_file = path + "/{}.{}".format(
                                instr, extension
                            )
                        ...
                        if length == -1:
                            print(
                                'Cant find file "{}" in folder {}'.format(instr, path)
                            )
                            continue
                ...
                for instr in self.instruments:
                    ...
                    if type(data_path) == list:
                        ...
                        track_paths += sorted(
                            glob(data_path + "/{}/*.flac".format(instr))
                        )
...
def download_musdb():
    root = p.Datasets
    train_data = MUSDB_HQ(root=root, download=True, subset="train")
    test_data = MUSDB_HQ(root=root, download=True, subset="test")
    zip_file = p.Datasets / "musdb18hq.zip"

    if zip_file.exists():
        zip_file.unlink()

    data_path = p.Datasets / "musdb18hq"

    if not p.chk_valid(data_path):
        train_dir = data_path / "train"
        valid_dir = data_path / "valid"
        valid_dir.mkdir(parents=True, exist_ok=True)

        train_tracks, valid_tracks = train_test_split(
            train_data, test_size=0.2, random_state=42
        )
        print("Moving validation tracks")
        for track in valid_tracks:
            track_name = track[3]
            track_path = train_dir / track_name
            shutil.move(
                track_path, os.path.join(valid_dir, os.path.basename(track_path))
            )

        train_data.tracks = train_tracks
...
def load_data(model_config, cfg, batch_size):
    ...
    cfg.data_path = 'Datasets/musdb18hq/valid'
    validset = MSSDataset(
            model_config,
            cfg.data_path,
            batch_size=batch_size,
            metadata_path=os.path.join(cfg.results_path, f'metadata_valid.pkl'),
            dataset_type=cfg.dataset_type,
        )
    cfg.data_path = 'Datasets/musdb18hq/test'
    testset = MSSDataset(
            model_config,
            cfg.data_path,
            batch_size=batch_size,
            metadata_path=os.path.join(cfg.results_path, f'metadata_test.pkl'),
            dataset_type=cfg.dataset_type,
        )
